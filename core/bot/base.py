...
class Bot:
    ...
    async def _update_account_proxy(self, attempt: int, max_attempts: int) -> None:
        ...
        if config.application_settings.disable_auto_proxy_change is False:
            ...
            if not self._db_account:
                logger.info(
                    f"账户: {self.account_data.wallet_address} | 代理已更改 | "
                    f"在 {error_delay}秒后重试.. | 尝试: {min(attempt + 1, max_attempts)}/{max_attempts}.."
                )
        ...
        logger.info(
            f"账户: {self.account_data.wallet_address} | {msg} | "
            f"在 {error_delay}秒后重试.. | 尝试: {min(attempt + 1, max_attempts)}/{max_attempts}.."
        )
        await asyncio.sleep(error_delay)
    ...
    async def get_captcha(wallet_address: str, captcha_type: Literal["cf", "geetest"], action: str = None) -> Optional[str]:
        ...
        async def handle_turnistale() -> Optional[str]:
            logger.info(f"账户: {wallet_address} | 正在解决 Cloudflare 验证码 | 尝试: {attempt + 1}/{max_attempts}")
            ...
            if success:
                logger.success(f"账户: {wallet_address} | Cloudflare 验证码已解决")
                return result

            raise ValueError(f"{result}")

        async def handle_geetest() -> Optional[str]:
            logger.info(f"账户: {wallet_address} | 正在解决 Geetest 验证码 | 尝试: {attempt + 1}/{max_attempts}")
            ...
            if success:
                logger.success(f"账户: {wallet_address} | Geetest 验证码已解决")
                return result
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.error(
                    f"账户: {wallet_address} | 解决 Cloudflare 验证码时发生错误: {str(e)} | 正在重试..."
                )
                if attempt == max_attempts - 1:
                    raise CaptchaSolvingFailed(f"在 {max_attempts} 次尝试后未能解决 Cloudflare 验证码")
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if config.web3_settings.verify_balance:
                    ...
                    threshold = config.web3_settings.faucet_balance_threshold
                    if balance >= threshold: # <-- Changed to threshold
                        logger.success(f"账户: {db_account_value.wallet_address} | 余额充足 ({balance} IRYS) (门槛: {threshold} IRYS) | 跳过水龙头")
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在从水龙头请求代币")
                await api.call_faucet(captcha_token=captcha_token, wallet_address=db_account_value.wallet_address)

                logger.success(f"账户: {db_account_value.wallet_address} | 代币已请求")
                return operation_success(self.account_data.private_key)

            except APIError as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"账户: {self.account_data.wallet_address} | 达到最大尝试次数，无法从水龙头请求代币 | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                if error.error_type == APIErrorType.INVALID_CAPTCHA:
                    logger.error(f"账户: {self.account_data.wallet_address} | 无效的验证码令牌")
                    await self._update_account_proxy(attempt, max_attempts)
                    continue

                logger.error(f"账户: {self.account_data.wallet_address} | 请求代币时发生错误 (APIError): {error} | 永久跳过")
                return operation_failed(self.account_data.private_key)
                ...
                logger.error(f"账户: {self.account_data.wallet_address} | 请求代币时发生错误 (通用异常): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_top_up_game_balance(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在准备充值游戏余额..")
                ...
                if config.web3_settings.verify_balance:
                    ...
                    threshold = config.web3_settings.top_up_balance_threshold
                    if play_balance >= threshold: # <-- Changed to threshold
                        logger.success(f"账户: {db_account_value.wallet_address} | 游戏余额充足 ({play_balance} IRYS) (门槛: {threshold} IRYS) | 跳过充值")
                        return operation_success(self.account_data.private_key)
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在充值 {top_up_amount} IRYS 到游戏余额..")

                irys_balance = await irys_games_module.human_balance()
                if irys_balance < top_up_amount + 0.001:
                    logger.error(f"账户: {db_account_value.wallet_address} | IRYS 余额不足以充值游戏余额 | 可用: {irys_balance} IRYS | 需要: {top_up_amount + 0.001} IRYS | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                success, result = await irys_games_module.deposit_tokens(amount=top_up_amount)
                if success:
                    logger.success(f"账户: {db_account_value.wallet_address} | 游戏余额已充值 | 交易: {self.tx_hash_to_explorer_link(result)}")
                    return operation_success(self.account_data.private_key)
                else:
                    logger.error(f"账户: {db_account_value.wallet_address} | 充值游戏余额失败: {result} | 永久跳过")
                    return operation_failed(self.account_data.private_key)

            except APIError as error:
                logger.error(f"账户: {self.account_data.wallet_address} | 充值余额时发生错误 (APIError): {error} | 永久跳过")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"账户: {self.account_data.wallet_address} | 达到最大尝试次数，无法充值游戏余额 | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"账户: {self.account_data.wallet_address} | 充值余额时发生错误 (通用异常): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_mint_omnihub_nft(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在准备铸造 Omnihub NFT..")
                ...
                if balance < 0.001:
                    logger.error(f"账户: {db_account_value.wallet_address} | IRYS 余额不足以铸造 NFT | 可用: {balance} IRYS | 至少需要: 0.001 IRYS | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                logger.info(f"账户: {db_account_value.wallet_address} | 正在铸造 Omnihub NFT..")
                success, result = await irys_omnihub_module.mint_nft()
                if success:
                    logger.success(f"账户: {db_account_value.wallet_address} | Omnihub NFT 已铸造 | 交易: {self.tx_hash_to_explorer_link(result)}")
                    return operation_success(self.account_data.private_key)
                else:
                    logger.error(f"账户: {db_account_value.wallet_address} | 铸造 Omnihub NFT 失败: {result} | 永久跳过")
                    return operation_failed(self.account_data.private_key)

            except APIError as error:
                logger.error(f"账户: {self.account_data.wallet_address} | 铸造 Omnihub NFT 时发生错误 (APIError): {error} | 永久跳过")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"账户: {self.account_data.wallet_address} | 达到最大尝试次数，无法铸造 Omnihub NFT | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"账户: {self.account_data.wallet_address} | 铸造 Omnihub NFT 时发生错误 (通用异常): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_wait_for_balance(self):
        ...
        try:
            ...
            logger.info(f"账户: {db_account_value.wallet_address} | 正在等待 IRYS 余额..")
            wallet = Web3Wallet(
                private_key=db_account_value.private_key,
                rpc_url=config.web3_settings.irys_rpc_url,
                proxy=db_account_value.active_account_proxy
            ...
            while True:
                balance = await wallet.human_balance()
                if balance > 0:
                    logger.success(f"账户: {db_account_value.wallet_address} | 余额充足 ({balance} IRYS) | 继续..")
                    return operation_success(self.account_data.private_key)

                elapsed = time.monotonic() - start
                if elapsed >= TIME_LIMIT:
                    logger.error(f"账户: {db_account_value.wallet_address} | 等待 IRYS 余额超时 {TIME_LIMIT}秒")
                    return operation_failed(self.account_data.private_key)

                remaining = int(TIME_LIMIT - elapsed)
                sleep_for = min(CHECK_INTERVAL, remaining)
                logger.info(f"账户: {db_account_value.wallet_address} | 余额不足 ({balance} IRYS) | 正在 {int(sleep_for)}秒后再次检查..")
                await asyncio.sleep(sleep_for)

        except Exception as error:
            error = validate_error(error)
            logger.error(f"账户: {self.account_data.wallet_address} | 等待余额时发生错误 (通用异常): {error} | 永久跳过")
            return operation_failed(self.account_data.private_key)
    ...
    async def process_all_in_one(self) -> OperationResult | None:
        ...
        try:
            ...
            tasks_to_execute = list(config.all_in_one_settings.tasks_to_perform)
            if config.all_in_one_settings.shuffle_tasks:
                random.shuffle(tasks_to_execute)
                logger.info(f"账户: {db_account_value.wallet_address} | 一体化任务顺序已打乱。")
            ...
            for task in tasks_to_execute:
                if task == "faucet":
                    ...
                    logger.error(f"账户: {db_account_value.wallet_address} | 未知任务: {task} | 跳过")
                    continue

                if operation_result is None or operation_result.get("status") is False:
                    logger.error(f"账户: {db_account_value.wallet_address} | 任务 {task} 失败 | 永久跳过")
                    return operation_failed(self.account_data.private_key)
                else:
                    logger.success(f"账户: {db_account_value.wallet_address} | 任务 {task} 已完成")
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在等待 {delay}秒后开始下一个任务..")
                await asyncio.sleep(delay)

            logger.success(f"账户: {db_account_value.wallet_address} | 所有任务已完成")
            return operation_success(self.account_data.private_key)

        except Exception as error:
            error = validate_error(error)
            logger.error(f"账户: {self.account_data.wallet_address} | 完成任务时发生错误 (通用异常): {error} | 永久跳过")
            return operation_failed(self.account_data.private_key)
    ...
    async def process_play_games(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"账户: {db_account_value.wallet_address} | 正在准备玩游戏..")
                irys_games_module = IrysGamesModule(
                    private_key=db_account_value.private_key,
                    rpc_url=config.web3_settings.irys_rpc_url,
                    proxy=db_account_value.active_account_proxy
                ...
                play_balance = await irys_games_module.get_play_balance()
                if play_balance > 0:
                    logger.success(f"账户: {db_account_value.wallet_address} | 游戏余额充足 ({play_balance} IRYS)")
                else:
                    logger.info(f"账户: {db_account_value.wallet_address} | 游戏余额不足 ({play_balance} IRYS) | 请先充值游戏余额 | 永久跳过")
                    return operation_failed(self.account_data.private_key)
                ...
                for game in config.games_settings.games_to_play:
                    if game in completed_games:
                        logger.info(f"账户: {db_account_value.wallet_address} | 游戏 {game} 已完成 | 跳过")
                        continue
                    ...
                    logger.info(f"账户: {db_account_value.wallet_address} | 正在等待 {delay}秒后开始下一个游戏..")
                    await asyncio.sleep(delay)

                logger.success(f"账户: {db_account_value.wallet_address} | 所有游戏已完成")
                return operation_success(self.account_data.private_key)

            except APIError as error:
                logger.error(f"账户: {self.account_data.wallet_address} | 玩游戏时发生错误 (APIError): {error} | 永久跳过")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"账户: {self.account_data.wallet_address} | 达到最大尝试次数，无法玩游戏 | 永久跳过")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"账户: {self.account_data.wallet_address} | 玩游戏时发生错误 (通用异常): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def execute_game(self, game: GameType, api: IrysAPI, irys_games_module: IrysGamesModule, db_account_value: Accounts) -> None:
        logger.info(f"账户: {db_account_value.wallet_address} | 正在开始游戏: {game}")
        ...
        logger.info(f"账户: {db_account_value.wallet_address} | 游戏 {game} 已开始 | 交易: {tx} | 正在等待 {delay}秒后完成..")
        await asyncio.sleep(delay)
        ...
        tx = self.tx_hash_to_explorer_link(response["transactionHash"])
        logger.success(f"账户: {db_account_value.wallet_address} | 游戏 {game} 已完成 | 分数: {score} | 交易: {tx}")


    @staticmethod
    async def execute_sprite_type_game(api: IrysAPI, db_account_value: Accounts):
        logger.info(f"账户: {db_account_value.wallet_address} | 正在开始游戏: Sprite Type")
        game_stats = generate_sprite_game_stats()

        anti_cheat_hash = generate_anti_cheat_hash(db_account_value.wallet_address, game_stats)
        logger.info(f"账户: {db_account_value.wallet_address} | 已生成游戏统计数据和反作弊哈希")

        logger.info(f"账户: {db_account_value.wallet_address} | 正在提交游戏结果到服务器..")
        ...
        url = response["url"]
        logger.success(f"账户: {db_account_value.wallet_address} | Sprite Type 游戏已完成 | 查看结果: {url}")
